...
SELECT DISTINCT p.apellido1, p.apellido2, nombre, telefono FROM persona p WHERE tipo = 'alumno' HAVING telefono IS NULL;
...
SELECT DISTINCT p.apellido1, p.apellido2, nombre, telefono FROM persona p WHERE tipo = 'alumno' AND p.nif LIKE '%K' HAVING telefono IS NULL ;
